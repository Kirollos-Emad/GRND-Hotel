...
<html>
    <head>
        <title>Enable guest account</title>
        ...
        <script>
            ...
            function Enable(id)
            {
                let text = "By clicking ok the guest account will be Enabled";
                
                if (confirm(text) == true) 
                {
                    jQuery.ajax({
                        url: "includes/func/enable_guest_account.php",
                        data: 'id=' + id,
                        type: "POST",
                        success: function(data){
                            location.reload();
    ...
    <body>
        ...
        <h1 style = "border: 6px solid gray; background-color: #f0f0f0; width: 100%; text-align: center;">
            Please click on the image or the name to enable guest account 
            </h1>
            ...
            <?php
                $sql_get_rec_data = "select UserId, UserFirstName, UserLastName,UserImage,UserMail FROM tbl_user WHERE UserAccountStatus = 'deactivated' AND UserRole = 'guest'";
