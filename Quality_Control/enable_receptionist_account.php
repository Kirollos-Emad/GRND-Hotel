...
<html>
    <head>
        <title>Enable Receptionist account</title>
        ...
        <script>
            ...
            function Enable(id)
            {
                let text = "By clicking ok the receptionist account will be Enabled";
                
                if (confirm(text) == true) 
                {
                    jQuery.ajax({
                        url: "includes/func/enable_receptionist_account.php",
                        data: 'id=' + id,
                        type: "POST",
                        success: function(data){
                            location.reload();
    ...
    <body>
        ...
        <h1 style = "border: 6px solid gray; background-color: #f0f0f0; width: 100%; text-align: center;">
            Please click on the image or the name to enable receptionist account 
            </h1>
            ...
            <?php
                $sql_get_rec_data = "select UserId, UserFirstName, UserLastName,UserImage,UserMail FROM tbl_user WHERE UserAccountStatus = 'deactivated' AND UserRole = 'receptionist'";
                ...
                if($result_rec_data->num_rows > 0)
                {
                    while($row_rec_data = $result_rec_data->fetch_array())
                    {
                        echo "<div class = \"card\">
                                ...
                                <div class = \"buttons\">
                                    <input type = \"button\" Value = \"Enable\" onclick = \"Enable(".$row_rec_data['UserId'].")\">
                                </div>
                            </div>";
                    }
                }
